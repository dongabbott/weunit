...
def get_token(url, headers, data, path):
    data = WeHttpClient(url, params=data, headers=headers).send(method='post')
    try:
        result = data.json()
        for x in path.split('.'):
            result = result.get(x)
...
def logout(url, headers):
    data = WeHttpClient(url, headers=headers).send(method='post')
    try:
        result = data.json()
        return result
    except Exception as e:
